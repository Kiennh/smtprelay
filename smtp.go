...
func SendMail(r *Remote, from string, to []string, msg []byte) error {
	...
	if re.Match(msg) {
		...
		s = re2.ReplaceAllString(string(msg), fmt.Sprintf("From: %s <%s>\nTo:", strings.Split(from, "@")[0], from))
	}
	if r.Name != "" {
		s = re.ReplaceAllString(s, fmt.Sprintf("From: %s <$2>", r.Name))
	}
	msg = []byte(s)
...
func (a *loginAuth) Next(fromServer []byte, more bool) ([]byte, error) {
	if more {
		...
		default:
			return nil, errors.New("UNKNOWN ERROR FROM SERVER")
		}
	}
	return nil, nil
}
